...
def feedforward(model):
    ...
    with torch.no_grad():
        ...
        a = torch.argmax(model(torch.cuda.FloatTensor(s.flatten()))).item()
...
def backpropagate(model, criterion, optimizer, r, s, s1, game_over, r_list):
    ...
    a1 = model(torch.cuda.FloatTensor(s1.flatten()))
...
agentB = QNetwork(state_space, action_space).to(device)
...
for i in range(num_episodes):
    if i % 100 == 0:
        ...
        print("\Average steps per episode: " + str(sum(jList)/num_episodes), flush=True)
        # print("\nScore over time A: " + str(sum(rAList)/num_episodes), flush=True)
        print("\nScore over time B: " + str(sum(rBList)/num_episodes), flush=True)
    ...
    while game.game_over == False:
        ...
        if game.player == True:
            ...
            r, s1, game_over = game.perform_action(np.random.randint(7))
            ...
            s, rB, game_over = backpropagate(agentB, criterion, optimizerB, r[0], s, s1, game_over, rB)
